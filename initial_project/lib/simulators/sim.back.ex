...
defmodule ImtSim.Back do
  ...
  def init(_) do
    children = [
      ImtSim.Back.Stocks,
      ImtSim.Back.Stats,
      {Plug.Cowboy, scheme: :http, plug: ImtSim.Back.OrderReceiver, options: [port: @order_receiver_port]}
    ]
...
defmodule ImtSim.Back.OrderReceiver do
  ...
  post "/order/new" do
    create_order_duration = :timer.seconds(2) + (:rand.uniform(8) * 2) # random duration between 2s and 4s with 0.25s step.
