...
defmodule ReqSender do
  ...
  def handle_info(:send_req, %{running: false} = state) do
    {:noreply, state}
  end
  ...
  def maybe_end(%{running: false, c: 0, parent: _parent} = state) do
    :init.stop()
    state
  end

  def maybe_end(%{c: c} = state) when rem(c, 10000) == 0 do
    IO.write(:stderr, ".")
    state
  end

  def maybe_end(state) do
    state
  end
end
...
defmodule Req do
  ...
  def get_req_ko400(id, path, logfile) do
    ...
    IO.write(
      logfile,
      "#{id},#{ts},#{String.replace(path, ",", "-")},#{div(time, 1000)},#{ok?},#{other}\n"
    )
  end
  ...
  def post_random_order(req_id, logfile) do
    order =
      Poison.encode!(%{
        id: "#{req_id}",
        products: [
          %{id: :rand.uniform(10), quantity: :rand.uniform(5)},
          %{id: :rand.uniform(10), quantity: :rand.uniform(5)}
        ]
      })
    ...
    spawn(fn ->
      # le paiement arrive après la commande
      :timer.sleep(2_000 + :rand.uniform(16) * 125)
  ...
  def post_payment(order_id, logfile) do
    ...
    {time, {ok?, other}} =
      :timer.tc(fn ->
        case :httpc.request(
               :post,
               {'#{@url}/order/#{order_id}/payment-callback', [], 'application/json',
                transaction},
               [],
               []
             ) do
    ...
    IO.write(
      logfile,
      "#{900_000_000 + order_id},#{ts},/order/#{order_id}/payment-callback',#{div(time, 1000)},#{ok?},#{other}\n"
    )
  end
end
    ...
    children = [
      {
        ...
        todo: [
          {1, &Req.get_req_ko400(&1, "/aggregate-stats/#{:rand.uniform(10)}", logfile)},
