defmodule LoadBalancer do
  ...
  def init(_) do
    ...
    nb_instances = Application.fetch_env!(:imt_order, :transactors) |> Enum.count()
    Logger.info("[#{Node.self()}] [LoadBalancer] Started Load Balancer on #{port}")

    instances =
      for i <- 1..nb_instances do
        {{127, 0, 0, 1}, 9093 + i}
      end
    ...
    Supervisor.init(
      [
        ...
        {LoadBalancer.Proxy.Listener,
         port: port, ip: {127, 0, 0, 1}, checkout_mod: LoadBalancer.RoundRobin}
      ],
...
defmodule LoadBalancer.RoundRobin do
  ...
  def handle_call(:checkout, _, {[h], acc}) do
    {:reply, h, {Enum.reverse([h | acc]), []}}
  end

  def handle_call(:checkout, _, {[h | t], acc}) do
    {:reply, h, {t, [h | acc]}}
  end
...
defmodule LoadBalancer.Proxy do
  ...
  defmodule Listener do
    ...
    def init(opts) do
      ...
      {:ok, listen_sock} =
        :gen_tcp.listen(opts[:port], [:binary, active: true, ip: opts[:ip] || :any])

      for _ <- 1..acceptor_num do
        spawn_link(fn -> acceptor(listen_sock, opts[:checkout_mod]) end)
      end
  ...
  def init({s, {ip, port} = backend} = args) do
    {:ok, backend_s} =
      case :gen_tcp.connect(ip, port, [:binary, packet: 0, active: true]) do
        {:error, err} ->
          IO.puts("Error while trying to connect to: #{inspect({ip, port})}:   #{inspect(err)}")
          :timer.sleep(:timer.seconds(5))
          {:ok, %{backend_s: backend_s}, _} = init(args)
          {:ok, backend_s}
  ...
  def handle_info({tcp_die, s}, %{backend_s: s} = state)
      when tcp_die in [:tcp_closed, :tcp_error] do
